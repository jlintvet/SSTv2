...
Retrieves the last three days of SST data from two satellite sources:
  ...
  NOTE: A second near-real-time dataset will be added once a verified
  active ERDDAP endpoint for GOES/VIIRS is confirmed.
...
def _parse_acspo_csv(text: str) -> list[dict]:
    """
    Parse ERDDAP csvp response for ACSPO L3S SST.
    Variable: sea_surface_temperature in Celsius.
    NaN / fill values indicate cloud-covered pixels — stored as null.
    """
    reader   = csv.reader(io.StringIO(text))
    rows_raw = list(reader)
    if len(rows_raw) < 3:
...
def main():
    ...
    log.info("=== MUR + ACSPO SST Daily Retrieval ===")
