#!/usr/bin/env python3
"""
Pre-bake the ocean mask for the Virginia-to-Rhode Island region.

Output: DailySSTData/va_ri/ocean_mask.json
...
def main():
    ...
    for poly in polys:
        ...
        if mxl >= WEST and mnl <= EAST and mxla >= SOUTH and mnla <= NORTH:
            kept.append(poly)
    print(f"  {len(polys)} total polygons, {len(kept)} intersect VA-RI region")
