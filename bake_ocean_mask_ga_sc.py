#!/usr/bin/env python3
"""
Pre-bake the ocean mask for the GA/SC region.

Output: DailySSTData/ga_sc/ocean_mask.json
...
def main():
    ...
    for poly in polys:
        ...
        if mxl >= WEST and mnl <= EAST and mxla >= SOUTH and mnla <= NORTH:
            kept.append(poly)
    print(f"  {len(polys)} total polygons, {len(kept)} intersect GA/SC region")
