...
### 2026-07-05 — Job-timeout data loss: soft time budget + per-region commits
`Daily SST.yml` runs `timeout-minutes: 60` for the whole job. The "Fetch SST data" step ran both regions sequentially, then a separate "Commit SST data to repo" step committed everything at the very end. GitHub kills an overrunning job with no warning and no chance to run later steps -- so a run that overran while processing the *second* region lost data from the *first* region too, even though it had already fetched successfully. Nothing gets committed on a kill. This is very likely why the frontend showed no new hourly SST from 2026-07-03 ~12pm onward, alongside repeated "exceeded the maximum execution time of 1h0m0s" failures. Two fixes, deployed together:

1. **Soft time budget in `sst_data_fetcher.py`:** added `PIPELINE_START_MONO` (captured at import time) and `SOFT_TIME_BUDGET_S = 25 * 60`. The VIIRS fetch loop (by far the most expensive phase — up to ~144 requests/region across 3 platforms x 48 hourly windows) checks elapsed time before each granule attempt and breaks out of all three loop levels (platform / day / granule) once the budget is hit, via a `budget_hit` flag. `main()` then finishes normally and prints "Pipeline complete" instead of being killed mid-request. 25 min x 2 regions = 50 min, leaving ~10 min of the 60-min cap for checkout, dependency install, and verify/commit steps.
2. **Per-region commits in the workflow:** moved the `git add`/`commit`/`push` logic from the standalone end-of-job step into the shell `for REGION in $REGIONS` loop in `Daily SST.yml`, so each region's `DailySSTData/` changes are committed and pushed immediately after that region's script finishes — before the next region starts. Removed the now-redundant standalone "Commit SST data to repo" step. The "Commit fetch log to repo" step is unchanged (still runs once at the end via `if: always()` — losing the diagnostic log on a kill is a minor loss, unlike losing the actual SST data).

Together: the time budget stops each region well inside its own share of the 60-minute cap, and per-region commits mean that even if something still runs long, only that region's data is at risk — never a region that already finished. Verified the nested-break logic with a standalone loop simulation and the patched YAML with PyYAML before deploying either.
