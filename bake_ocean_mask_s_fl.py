...
"""
Pre-bake the ocean mask for the Southern Florida region.

Output: DailySSTData/s_fl/ocean_mask.json
...
def main():
    ...
    for poly in polys:
        ...
        if mxl >= WEST and mnl <= EAST and mxla >= SOUTH and mnla <= NORTH:
            kept.append(poly)
    print(f"  {len(polys)} total polygons, {len(kept)} intersect Southern FL region")
